...
def go_back():
    ...
    # clear deeper state as we go back
    if p == 4:
...
# Page 1: Models
    ...
    if sel=="All": models=get_models(brand)
    else:
        with get_engine().connect() as conn:
            rs=conn.execute(text("SELECT DISTINCT model FROM pdf_info WHERE brand=:b AND year=:y"),{"b":brand,"y":int(sel)}).fetchall()
    ...
    for col,m in zip(cols[:-1],models):
        with col:
            row=get_engine().connect().execute(
                text("SELECT bike_image FROM pdf_info WHERE brand=:b AND model=:m LIMIT 1"),{"b":brand,"m":m}
            ).fetchone()
            blob=row[0] if row and row[0] else None
            if blob:
                raw=bytes(blob) if isinstance(blob,memoryview) else blob
                try: img=process_image(raw,size)
