...
.styles_viewerBadge__1yB5_, .viewerBadge_link__1S137, .viewerBadge_text__1JaDK{ display: none; } #MainMenu{ visibility: hidden; }
...
if "img_cache" not in st.session_state:
    st.session_state.img_cache = {}  # maps (id, size) → PIL.Image


def process_image(img_bytes: bytes, size: tuple[int,int]):
    """
    Resize the raw JPEG/PNG bytes to size, but only once per unique
    image+size during this session. Returns a PIL.Image.
    """
...
def add_to_cart(part, widget_key):
    ...
    _save_cart_cookie()
    st.session_state.show_qr = False  # show the cart
...
# Ensure old carts get new fields
cart = st.session_state.cart_data
