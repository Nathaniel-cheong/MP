...
if st.session_state.view == "cart":
    ...
    with st.form("checkout_form"):
        ...
        if submit_clicked:
            ...
            meta = MetaData()
            meta.reflect(bind=engine, only=["master_parts_list"])
            mpl = Table("master_parts_list", meta, autoload_with=engine)
            eb  = Table(
                "ebasket", meta,
                ...
                Column("mpl_id", Integer,
                       ForeignKey("master_parts_list.mpl_id"),
                       nullable=False),
            ...
            with engine.connect() as conn:
                rd = conn.execute(select([mpl.c.part_no, mpl.c.mpl_id])).fetchall()
            part_to_id = {r[0]: r[1] for r in rd}
